				...
				foreach ($arSource as $key => $arProperties)
				{
					...
					<div class="line" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						elseif ($arProperties["TYPE"] == "TEXT")
						{
							...
							<?php
							$emailMaskClass = ($arProperties["CODE"] == "email") ? "email_mask" : "";
							?>

							<?php if ($arProperties["CODE"] == "phone"): ?>
								<div class="phone_mask_mode">
									...
									<input class="input phone_mask" type="text"
										   value="<?= $arProperties["VALUE"] ?>"
										   name="<?= $arProperties["FIELD_NAME"] ?>"
										   id="<?= $arProperties["FIELD_NAME"] ?>"/>
								</div>
							<?php else: ?>
								<input class="input <?= $emailMaskClass ?>" type="text" maxlength="<?= ($arProperties["MAXLENGTH"]) ? $arProperties["MAXLENGTH"] : "250" ?>" size="<?= $arProperties["SIZE1"] ?>"
