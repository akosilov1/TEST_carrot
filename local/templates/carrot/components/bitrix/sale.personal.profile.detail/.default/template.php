...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post"  class="col-md-12 sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		...
		<div class="form-group row">
			<label class="col-sm-2 col-form-label sale-personal-profile-detail-form-label "><?=Loc::getMessage('SALE_PERS_TYPE')?></label>
			<div class="col-sm-10">
				<?=$arResult["PERSON_TYPE"]["NAME"]?>
			</div>
		</div>		
		<div class="form-group row">
			<label class="col-sm-2 col-form-label sale-personal-profile-detail-form-label " for="sale-personal-profile-detail-name">
			...
			if (!empty($block["PROPS"]))
			{
				...
				foreach($block["PROPS"] as $key => $property)
				{
					...
					<div class="form-group row sale-personal-profile-detail-property-<?=strtolower($property["TYPE"])?>">
						<label class="col-sm-2 col-form-label sale-personal-profile-detail-form-label  <?=$alignTop?>" for="sppd-property-<?=$key?>">
							<?= $property["NAME"]?>:
							<?
